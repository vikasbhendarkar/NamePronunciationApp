...
def insert_calculation(firstNum, secondNum, answer):
    ...
    #     engine.dispose()
    DICT.append(
        {"first_operand": firstNum, "second_operand": secondNum, "answer": answer}
    )
    return "Insertion successful"
...
def get_calculations():
    ...
    try:
        ...
        calculations = DICT_DB

        calculations_history = {}

        i = 1
        for row in calculations:
            calculations_history[i] = (
                row["first_operand"],
                row["second_operand"],
                row["answer"],
            )
...
def delete_calculation(firstNum, secondNum):
    ...
    try:
        engine = create_engine(URI, echo=False)
        my_connection = engine.connect()
        my_connection.execute(
            "DELETE from calculations WHERE first_operand = {} AND second_operand = {}".format(
                firstNum, secondNum
            )
        )
        return "Deletion successful"
    except exc.SQLAlchemyError as err:
        return "Error deleting data from table {}. Exception: {}".format(
            "calculations", err
        )
